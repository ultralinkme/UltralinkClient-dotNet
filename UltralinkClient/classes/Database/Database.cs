...
namespace UL
{
    public class Database : ULBase
    {
        ...
        public Ultralink ulFromURL( string URL, string trimset = "" ){ string ulID = APICall(new JObject{ ["ulFromURL"] = URL, ["trimset"] = trimset }, "Could not lookup the Ultralink for URL " + URL).ToString(); if( ulID != "0" ){ return Ultralink.U( ulID ); } return null; }
        ...
        public Ultralink ulFromWord( string word, bool caseSensitive = false, string category = "", bool recent = false){ string ulID = APICall(new JObject{ ["ulFromWord"] = word, ["case"] = caseSensitive, ["category"] = category, ["recent"] = recent }, "Could not lookup the Ultralink for word " + word).ToString(); if( ulID != "0" ){ return Ultralink.U( ulID ); } return null; }
